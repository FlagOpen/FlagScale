...
# Function to find FlagScale root directory
find_flagscale_root() {
    CURRENT_DIR=$(pwd)
    while [ "$CURRENT_DIR" != "/" ]; do
        if [ -f "$CURRENT_DIR/run.py" ] || [ -d "$CURRENT_DIR/.git" ]; then
            echo "$CURRENT_DIR"
            return
        fi
        CURRENT_DIR=$(dirname "$CURRENT_DIR")
    done
    echo "FlagScale root not found!" >&2
    exit 1
}

# Save FlagScale root directory
FLAG_DIR=$(find_flagscale_root)
echo "FlagScale root directory: $FLAG_DIR"

get_device_type() {
    local script_path flag_dir relative_path device_path device_type

    # Path detection
    script_path=$(realpath "$0") || { echo "Error: Unable to parse script path"; exit 1; }
    flag_dir=$(find_flagscale_root) || { echo "Error: Cannot find flagscale root directory"; exit 1; }

    # Check if the script is located in the flagscale directory
    if [[ "$script_path" == "$flag_dir"/* ]]; then
        relative_path="${script_path#$flag_dir/}"
        device_path="${relative_path%%/tests/*}"
        device_type="${device_path#tests/}"
        if [[ -z "$device_type" ]]; then
            echo "Unable to determine device type" >&2
            exit 1
        fi
        echo "$device_type"
    else
        echo "The script is not in the flagscale directory" >&2
        exit 1
    fi
}

device=$(get_device_type)
echo "Start testing the functionality of ${device}"
