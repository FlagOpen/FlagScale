...
run_tests() {
    ...
    if [ "$_type" == "batch" ]; then
        ...
        torchrun --nproc_per_node=8 -m pytest --import-mode=importlib --cov-append --cov-report=html:/workspace/report/$id/cov-report-${backend} --cov=${backend}/${coverage} -q -x -p no:warnings $ignore_cmd $_test_files
        if [ $? -ne 0 ]; then
        ...
        for _test_file in $_test_files; do
            ...
            torchrun --nproc_per_node=8 -m pytest --import-mode=importlib --cov-append --cov-report=html:/workspace/report/$id/cov-report-${backend} --cov=${backend}/${coverage} -q -x -p no:warnings $ignore_cmd $_test_file
