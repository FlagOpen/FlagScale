...
# Check if the upstream remote already exists, otherwise add it
if ! git remote get-url upstream > /dev/null 2>&1; then
    git remote add upstream https://github.com/FlagOpen/FlagScale.git
fi

# Fetch the main branch from upstream
...
diff-cover "$report_dir/$coverage_file" --compare-branch=$common_ancestor --html-report "$report_dir/diff-cover-report-${backend}/index.html" --fail-under=70
