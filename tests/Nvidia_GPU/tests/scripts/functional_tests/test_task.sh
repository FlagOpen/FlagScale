...
test_task() {
  ...
  for _case in "${_cases[@]}"; do
    ...
    for attempt_i in $(seq 1 $test_times); do
      ...
      if [ "${_type}" = "serve" ]; then
         run_command "python $FLAG_DIR/run.py --config-path tests/${PWD##*/}/tests/functional_tests/test_cases/${_type}/${_task}/conf --config-name ${_case} action=test; sleep 1m" $attempt_i $_task $_type $_case
         ...
         run_command "python $FLAG_DIR/run.py --config-path tests/${PWD##*/}/tests/functional_tests/test_cases/${_type}/${_task}/conf --config-name ${_case} action=stop" $attempt_i $_task $_type $_case
      fi
