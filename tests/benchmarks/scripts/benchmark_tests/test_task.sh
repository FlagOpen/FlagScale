...
CONFIG_FILE="tests/benchmarks/scripts/benchmark_tests/config.yml"
...
test_task() {
  ...
  local _cases=$(python tests/benchmarks/scripts/benchmark_tests/parse_config.py --config $CONFIG_FILE --type $_type --task $_task)
  ...
  for _case in "${_cases[@]}"; do
    ...
    for attempt_i in $(seq 1 $test_times); do
      ...
      if [ "${_type}" = "train" ] || [ "${_type}" = "hetero_train" ]; then
        ...
        run_command "pytest tests/benchmarks/test_utils/test_result.py::test_train_equal --test_path=tests/benchmarks/test_cases --test_type=${_type} --test_task=${_task} --test_case=${_case}" $attempt_i $_task $_type $_case
      fi
      if [ "${_type}" = "inference" ]; then
        run_command "python run.py --config-path tests/benchmarks/test_cases/${_type}/${_task}/conf --config-name ${_case} action=test" $attempt_i $_task $_type $_case
        run_command "pytest -s tests/benchmarks/test_utils/test_result.py::test_inference_equal --test_path=tests/benchmarks/test_cases --test_type=${_type} --test_task=${_task} --test_case=${_case}" $attempt_i $_task $_type $_case
      fi
