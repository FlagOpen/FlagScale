...
def test_inference_pipline(test_path, test_type, test_task, test_case):
    ...
    for i in range(0, len(result_lines), 4):
        ...
        assert result_group[0] == gold_group[0], f"First line mismatch:\nResult: {result_group[0]}\nGold: {gold_group[0]}"
        ...
        for j in range(1, 4):
            ...
            if len(result_parts) == 2 and len(gold_parts) == 2:
                ...
                assert result_key == gold_key, f"Line {j+1} keys mismatch:\nResult: {result_group[j]}\nGold: {gold_group[j]}"
            else:
                print(f"Warning: One of the lines doesn't have an '=' character: \nResult: {result_group[j]}\nGold: {gold_group[j]}")
                
        result_parts = result_group[1].split('=')
        assert len(result_parts) >= 2, \
            f"len(result_parts) should be 2"
