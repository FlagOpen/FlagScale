            ...
            if deploy_config.get("prefill_decode_disaggregation", False):
                ...
                for j in range(d_num):
                    ...
                    if update_d_address != master_ip and len(nodes) > 1:
                        ...
                        node_cmd = f"{ids_env} && {vllm_command} --port {http_port} --gpu_memory_utilization 0.7 --kv-transfer-config '\\''{d_kv_config_formate_json}'\\''"
                        if docker_name:
                        ...
                        d_cmd = f"{ids_env} && {vllm_command} --port {http_port} --gpu_memory_utilization 0.7 --kv-transfer-config '\\''{d_kv_config_json}'\\''"
