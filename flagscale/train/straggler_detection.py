...
class StragglerDetectionWrapper:
    ...
    def __call__(self, dest_func):
        def wrapper(*args, **kwargs):
            ...
            _args = get_args()
            passed_warmup_stage = _args.curr_iteration > _args.straggler_detection_warmup_iterations
            user_specified_level = _args.straggler_detection_level
            generate_report = _args.generate_report if hasattr(_args, 'generate_report') else False
            if passed_warmup_stage and user_specified_level and user_specified_level == self.level:
                ...
                if generate_report:
                    _args.generate_report = False
                    report = straggler.Detector.generate_report()
                    rank = get_rank()
                    if rank == 0:
                        print(f"Rank {rank} GPUs relative perf: {report.gpu_relative_perf_scores}")
                        print(
                        ...
                        stragglers = report.identify_stragglers(
                            gpu_rel_threshold=0.7, gpu_indiv_threshold=0.7
                        )
                        for key, straggler_ids in stragglers.items():
                            if (
                                key == 'straggler_gpus_individual'
                                or key == 'straggler_gpus_relative'
                            ):
                                straggler_ranks = [
                                    str(straggler_id.rank) for straggler_id in straggler_ids
                                ]
                                if straggler_ranks:
                                    ...
                                    print(f'No rank is identified as as \'{key}\' straggler')
                            elif (
                                key == 'straggler_sections_individual'
                                or key == 'straggler_sections_relative'
                            ):
...
def shutdown_straggler_detector(user_specified_level):
    if user_specified_level > 0:
        if not straggler:
            raise RuntimeError(
                "The 'nvidia_resiliency_ext' module is required for straggler "
