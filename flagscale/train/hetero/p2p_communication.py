...
def warm_up_comm_group_hetero(config: ModelParallelConfig):
    ...
    to_send_tensor = torch.empty(
        ...
        device=(
            ...
            if "gloo" not in pp_groups[0].name()
...
def is_inter_mesh_comm(para_ctx: ParallelContext, comm_with_front_layer: bool):
    ...
    assert (
        comm_with_front_layer is not None
    ), "Specify Communication Direction Necessary"
    if comm_with_front_layer:
        ...
        for i in range(0, para_ctx._current_process_mesh_index):
            total_prev_pipeline_model_parallel_size += para_ctx._process_meshes[
                i
            ]._rank_generator.pp
        return (
            get_pipeline_model_parallel_rank()
            == total_prev_pipeline_model_parallel_size
        )
    else:
        ...
        for i in range(
            0,
            min(
                para_ctx._current_process_mesh_index + 1, len(para_ctx._process_meshes)
            ),
        ):
            total_current_pipeline_model_parallel_size += para_ctx._process_meshes[
                i
            ]._rank_generator.pp
        return (
            get_pipeline_model_parallel_rank()
            == total_current_pipeline_model_parallel_size - 1
        )


def recv_forward_hetero(
    tensor_shape: Shape, config: ModelParallelConfig
) -> torch.Tensor:
        ...
        if not is_inter_mesh_comm(para_ctx=para_ctx, comm_with_front_layer=True):
            ...
            if tensor_slices is not None:
                for tensor_slice in tensor_slices:
                    ...
                    input_tensor.data[sp_start:sp_end, dp_start:dp_end, :] = (
                        input_tensor_sliced
                    )
        if config.timers is not None:
        ...
        if not is_inter_mesh_comm(para_ctx=para_ctx, comm_with_front_layer=False):
            ...
            if tensor_slices is not None:
                for tensor_slice in tensor_slices:
                    ...
                    output_tensor_grad.data[sp_start:sp_end, dp_start:dp_end, :] = (
                        output_tensor_grad_sliced
                    )
        if config.timers is not None:
            config.timers("backward-recv").stop()

    if output_tensor_grad is not None and output_tensor_grad.device == torch.device(
        "cpu"
    ):
...
def send_forward_hetero(
    output_tensor: torch.Tensor, config: ModelParallelConfig
) -> None:
    ...
    if not core.parallel_state.is_pipeline_last_stage():
        ...
        if not is_inter_mesh_comm(para_ctx=para_ctx, comm_with_front_layer=False):
            ...
            _communicate(
                tensor_send_next=(
                    output_tensor if "gloo" not in group.name() else output_tensor.cpu()
                ),
                tensor_send_prev=None,
                recv_prev=False,
                recv_next=False,
            ...
            if tensor_slices is not None:
                for tensor_slice in tensor_slices:
                    ...
                    output_tensor_sliced = output_tensor[
                        sp_start:sp_end, dp_start:dp_end, :
                    ]
                    group = None
                    pp_groups = para_ctx.get_pipeline_model_parallel_group()
                    for pp_group in pp_groups:
                    ...
                    _communicate(
                        tensor_send_next=(
                            output_tensor_sliced.contiguous()
                            if "gloo" not in group.name()
                            else output_tensor_sliced.cpu()
                        ),
                        tensor_send_prev=None,
                        recv_prev=False,
                        recv_next=False,
...
def send_backward_hetero(
    input_tensor_grad: torch.Tensor, config: ModelParallelConfig
) -> None:
        ...
        if not is_inter_mesh_comm(para_ctx=para_ctx, comm_with_front_layer=True):
            ...
            if tensor_slices is not None:
                for tensor_slice in tensor_slices:
                    ...
                    input_tensor_grad_sliced = input_tensor_grad[
                        sp_start:sp_end, dp_start:dp_end, :
                    ]
                    group = None
                    pp_groups = para_ctx.get_pipeline_model_parallel_group()
                    ...
                    _communicate(
                        tensor_send_next=None,
                        tensor_send_prev=(
                            input_tensor_grad_sliced.contiguous()
                            if "gloo" not in group.name()
                            else input_tensor_grad_sliced.cpu()
                        ),
                        recv_prev=False,
                        recv_next=False,
                        tensor_shape=None,
