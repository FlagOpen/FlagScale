...
def find_overlapped_mapping(dim1, dim2, global_size=None):
    ...
    dim1_segments = [
        (i * dim1_segments_len, (i + 1) * dim1_segments_len) for i in range(dim1)
    ]
    dim2_segments_len = dim_lcm // dim2
    dim2_segments = [
        (i * dim2_segments_len, (i + 1) * dim2_segments_len) for i in range(dim2)
    ]
...
class ProcessMesh:
    ...
    def __init__(
        ...
        if virtual_pipeline_model_parallel_size is not None:
            ...
            self._virtual_pipeline_model_parallel_world_size = (
        ...
        self._all_group_ranks = defaultdict(
            list
        )  # group_ranks belongs to the current process mesh
        self._process_groups = {}  # process groups belongs to the current rank
        self._process_groups_gloo = (
            {}
        )  # process groups belongs to the current rank with gloo backend

        self.build_all_process_groups()

    def build_process_group(self, token, independent_ep=False, gloo=False):
        logical_ranks_list = self._rank_generator.get_ranks(
            token, independent_ep=independent_ep
        )
    ...
    def get_group_name(self, token, independent_ep=False):
        ...
        if independent_ep:
            ...
            assert (
                name is not None
            ), f"Token {token} does not support non-independent ep."
            return name
    ...
    def get_process_group(
        ...
        if check_initialized:
            assert group is not None, f"Process group {group_name} is not initialized."
        return group
    ...
    def get_process_group_size(self, token, independent_ep=False, gloo=False):
        ...
        if gloo:
            return torch.distributed.get_world_size(
                self._process_groups_gloo[group_name]
            )
        else:
    ...
    def get_process_group_ranks(
        ...
        if check_initialized:
            assert ranks is not None, f"Process group {group_name} is not initialized."
        return ranks
...
class ParallelContext:
    ...
    def build_all_process_meshes(self):
        ...
        for tp, cp, ep, dp, pp in self._args.hetero_process_meshes:
            process_mesh = ProcessMesh(
                ...
                order=(
                    "tp-usp-cp-ep-dp-pp"
                    if not self._args.use_tp_pp_dp_mapping
                    else "tp-pp-dp"
                ),
    ...
    def build_inter_mesh_process_groups(self, process_mesh1, process_mesh2):
        ...
        for s in range(sp1):
            ...
            for k in range(dp1):
                ...
                src_rank = process_mesh1.logical_coords_to_physical_ranks([src_coord])[
                    0
                ]
        ...
        for k in range(dp1):
            ...
            dst_dp_dims = [dim for dim, _, _ in dp_overlapped_mapping[k]]
            dst_coords = list(itertools.product([0], [0], dst_dp_dims, dst_pp_dims))
            src_rank = process_mesh1.logical_coords_to_physical_ranks([src_coord])[0]
            for dst_coord in dst_coords:
                dst_rank = process_mesh2.logical_coords_to_physical_ranks([dst_coord])[
                    0
                ]
    ...
    def build_global_process_groups(self):
        ...
        def _backtrack(mesh_index, prev_rank, path, token="pp", independent_ep=False):
            group_name = self._process_meshes[0].get_group_name(
                token, independent_ep=independent_ep
            )
            ...
            ranks_list = current_mesh.get_all_process_group_ranks(
                token, independent_ep=independent_ep, check_initialized=True
            )
            valid_ranks_list = []
            for ranks in ranks_list:
                ...
                for prev_path_ranks in path:
                    for prev_path_rank in prev_path_ranks:
                        if (
                            token == "pp"
                            and (prev_path_rank, ranks[0])
                            in self._inter_mesh_process_groups_pp
                        ):
                            mesh_is_connected = True
                        elif (
                            token == "tp-pp"
                            and (prev_path_rank, ranks[0])
                            in self._inter_mesh_process_groups_dp
                        ):
                            mesh_is_connected = True
        ...
        for mesh_index, process_mesh in enumerate(self._process_meshes):
            ...
            group = torch.distributed.new_group(
                ranks, timeout=self._timeout, use_local_synchronization=True
            )
            if self._rank in ranks:
        ...
        for ranks in self._group_ranks["pp"]:
            if len(ranks) > 1:
                ...
                if (
                    "pp_split" in self._parallel_ranks.keys()
                    and self._parallel_ranks["pp_split"] is not None
                ):
    ...
    def get_inter_mesh_process_group(self, src_rank, dst_rank):
        if (src_rank, dst_rank) in self._inter_mesh_process_groups_pp:
            ...
            raise RuntimeError(f"ProcessGroup [{src_rank}, {dst_rank}] does not exist.")
            ...
            for k in range(dp1):
                ...
                for i, dst_coord in enumerate(dst_coords):
                    ...
                    if (
                        ...
                        local_tensor_shape,
                        next,
                    ) not in self._inter_mesh_tensor_slices:
                        self._inter_mesh_tensor_slices[
    ...
    def is_pipeline_last_stage(self, ignore_virtual=False, group=None):
        ...
        if not ignore_virtual:
            ...
            if (
                ...
                return False
        return self.get_pipeline_model_parallel_rank(group) == (
    ...
    def is_pipeline_stage_at_split(self, group=None):
        ...
        decoder."""
        rank = self.get_pipeline_model_parallel_rank(group)
        return self.is_pipeline_stage_before_split(
            rank, group
        ) and self.is_pipeline_stage_after_split(rank + 1, group)
    ...
    def get_context_parallel_world_size(self):
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            return torch.distributed.get_world_size(
                group=self.get_context_parallel_group()
            )
        else:
            return 0
    ...
    def get_ulysses_sp_parallel_world_size(self):
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            return torch.distributed.get_world_size(
                group=self.get_ulysses_sp_parallel_group()
            )
        else:
            return 0
    ...
    def get_data_modulo_expert_parallel_rank(self, with_context_parallel):
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            return torch.distributed.get_rank(
                group=self.get_data_modulo_expert_parallel_group(
                    with_context_parallel=with_context_parallel
                )
            )
        else:
    ...
    def set_global_memory_buffer(self):
        ...
        assert (
            self._global_memory_buffer is None
    ...
    def get_global_memory_buffer(self):
        ...
        assert (
            self._global_memory_buffer is not None
