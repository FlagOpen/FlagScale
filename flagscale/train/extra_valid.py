...
from megatron.training.utils import is_last_rank, print_rank_0, print_rank_last
...
def build_extra_valid_datasets(build_extra_valid_dataset_provider):
    ...
    for num_tokens in num_tokens_list:
        assert (
            int(num_tokens) > 0
        ), f"Number of tokens {num_tokens} should be greater than 0"
    ...
    for path, num_samples, tag in zip(paths, num_samples_list, tags):
        ...
        extra_valid_datasets.append(
            build_extra_valid_dataset_provider(path, num_samples, tag)
        )
        ...
        if writer:
            ...
            if args.log_validation_ppl_to_tensorboard:
                ...
                if wandb_writer:
                    wandb_writer.log(
                        {
                            "{}/{} validation {} ppl".format(
                                ppl_section, key, label
                            ): ppl
                        },
                        iteration,
                    )
