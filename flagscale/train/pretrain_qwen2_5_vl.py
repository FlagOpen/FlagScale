...
def model_provider(
    ...
    # new_padded_vocab_size = args.padded_vocab_size   
    # print_rank_0(f"Building model with final vocab size: {new_padded_vocab_size}")
    ...
    # ==================== 步骤 1: 直接读取权威文件，计算最终词汇表大小 ====================
    
    print_rank_0("Step 1: Calculating vocab size directly from the new tokenizer.json file...")
    
    # 构建 tokenizer.json 文件的绝对路径
    tokenizer_json_path = os.path.join(args.tokenizer_path, 'tokenizer.json')
    ...
    try:
        ...
        # 从 JSON 数据中获取词汇表的大小
        new_vocab_size_raw = len(tokenizer_data['model']['vocab'])
    except (FileNotFoundError, KeyError) as e:
        raise RuntimeError(
            f"Could not read vocab size from {tokenizer_json_path}. "
            f"Please ensure this file exists and is a valid Hugging Face tokenizer file. Error: {e}"
        )

    make_divisible_by = args.make_vocab_size_divisible_by
    
    # 使用 Megatron 的标准对齐逻辑计算最终的 padded_vocab_size
    new_padded_vocab_size = (
        (new_vocab_size_raw + make_divisible_by - 1) // make_divisible_by
    ) * make_divisible_by

    print_rank_0(f"  > Raw vocab size from tokenizer.json: {new_vocab_size_raw}")
    print_rank_0(f"  > Padded vocab size (divisible by {make_divisible_by}): {new_padded_vocab_size}")

    # ==================== 步骤 2: 强制更新 args ====================
    
    print_rank_0("Step 2: Forcing args.padded_vocab_size to the new calculated size.")
    # 强制将我们计算出的新大小写入 args 对象，覆盖任何旧值
    args.padded_vocab_size = new_padded_vocab_size
    
    # ==================== 步骤 3: 创建模型 ====================

    print_rank_0("Step 3: Building the Qwen2.5-VL model...")
    # 这里的 config 和 model 创建过程会使用我们刚刚强制设置的、新的、大的 vocab_size
    config = core_transformer_config_from_args(args)
    use_te = args.transformer_impl == "transformer_engine"
    if not use_te:
        raise NotImplementedError("The Qwen2-VL model is only implemented with TransformerEngine!")

    if args.rotary_seq_len_interpolation_factor is not None and args.rotary_seq_len_interpolation_factor != 1:
        print_rank_0('Multimodal RoPE currently not support RoPE interpolation, set to None...')
        args.rotary_seq_len_interpolation_factor = None
    ...
    model = Qwen2_5VLModel(
        ...
        language_vocab_size=new_padded_vocab_size,
    ...
    log_dir = args.log_dir if hasattr(args, 'log_dir') and args.log_dir else './temp_logs'
    
    # 确保日志目录存在
    if rank == 0:
        os.makedirs(log_dir, exist_ok=True)
    # 加一个 barrier 确保 rank 0 创建完目录后，其他 rank 再继续
    torch.distributed.barrier()

    # 定义只属于这个 rank 的日志文件名
    log_file_path = os.path.join(log_dir, f"rank_{rank}_verification.log")

    # 使用 with open 确保文件被正确关闭
    with open(log_file_path, "w") as f:
        f.write(f"--- Verification Log for Rank {rank} ---\n")
        f.write(f"Timestamp: {datetime.now()}\n") # 可以加上时间戳
        f.write("=======================================\n")
        f.write(f"Args: {args}\n")
        f.write(f"New Padded Vocab Size from args: {new_padded_vocab_size}\n")
        f.write(f"Padded Vocab Size from args: {args.padded_vocab_size}\n")

        try:
            # 只有拥有该层的 rank 才会记录形状信息
            from megatron.core import parallel_state
            if parallel_state.is_pipeline_first_stage():
                input_embedding_shape = model.language_model.embedding.word_embeddings.weight.shape
                tp_size = parallel_state.get_tensor_model_parallel_world_size()
                f.write(f"This is a FIRST pipeline stage rank.\n")
                f.write(f"Actual Input Embedding Shape: {input_embedding_shape}\n")
                expected_dim0_on_this_rank = new_padded_vocab_size // tp_size

                # 进行断言
                assert input_embedding_shape[0] == expected_dim0_on_this_rank, \
                    f"Dimension mismatch! Actual shape[0] {input_embedding_shape[0]} != Expected on rank {expected_dim0_on_this_rank}"
                f.write("SUCCESS: Input embedding size matches args.\n")

            elif parallel_state.is_pipeline_last_stage():
                f.write(f"This is a LAST pipeline stage rank.\n")
                # 可以在这里添加对 output_layer 的验证
                if not model.language_model.share_embeddings_and_output_weights:
                    output_layer_shape = model.language_model.output_layer.weight.shape
                    f.write(f"Actual Output Layer Shape: {output_layer_shape}\n")
                    # ... 相应的断言 ...
                
            else:
                f.write(f"This is a MIDDLE pipeline stage rank.\n")
                f.write("No embedding/output layer to verify on this rank, which is correct.\n")

        except Exception as e:
            # 如果发生任何错误，都记录下来
            f.write(f"\n!!! AN ERROR OCCURRED ON THIS RANK !!!\n")
...
def add_multimodal_extra_args(parser):
    ...
    #                    help="A flag to ensure training starts from scratch, ignoring any load paths.")
