...
from flagscale.train.extra_valid import (
...
def pretrain(
    ...
    if "gloo" in torch.distributed.get_backend():
        start_time_tensor = torch.tensor(
            [_TRAIN_START_TIME], dtype=torch.double, device="cpu"
        )
    else:
        start_time_tensor = torch.tensor(
...
def train(
    ...
    if args.overlap_param_gather and args.align_param_gather:
        config.param_sync_func = [
            functools.partial(optimizer.start_param_sync, model_index)
    ...
    while iteration < args.train_iters:
        ...
        if args.extra_valid_interval and iteration % args.extra_valid_interval == 0:
            ...
            if args.virtual_pipeline_model_parallel_size is not None:
                ...
                extra_valid_data_iterators = build_extra_valid_data_iterators(
                    extra_valid_dataset_provider
                )
            ...
            if args.do_extra_valid:
                ...
                if args.manual_gc and args.manual_gc_eval:
                    ...
                    gc.collect()
                prefix = "iteration {}".format(iteration)
                for extra_valid_index, extra_valid_data_iterator in enumerate(
                    extra_valid_data_iterators
                ):
                    extra_evaluate_and_print_results(
                        extra_valid_index,
...
def evaluate(
    ...
    if extra_valid_index is not None:
        assert (
            args.extra_valid_iters_list is not None
        ...
        if writer:
            ...
            if args.log_validation_ppl_to_tensorboard:
                ...
                if wandb_writer:
                    ...
                    wandb_writer.log(
                        {
                            "{} validation ppl vs samples".format(
                                key
                            ): args.consumed_train_samples
                        },
                        iteration,
...
def build_train_valid_test_data_loaders(build_train_valid_test_datasets_provider):
    ...
    if is_distributed or mpu.get_tensor_model_parallel_rank() == 0:
        ...
        flags = torch.tensor(
            ...
            dtype=torch.long,
            device=get_device_type_for_comm(),
        )
    else:
        flags = torch.tensor(
            [0, 0, 0], dtype=torch.long, device=get_device_type_for_comm()
        )
