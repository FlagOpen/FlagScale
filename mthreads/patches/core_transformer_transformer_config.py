...
class TransformerConfig(ModelParallelConfig):
    ...
    recompute_attention_layernorm: bool = True
    recompute_attention_layernorm_layers: int = 1
...
for k in sys.modules:
    if k.startswith('megatron.core.transformer.transformer_config') or k.startswith('megatron.core.transformer'):
        for target in ['TransformerConfig']:
            if getattr(sys.modules[k], target, None):
                setattr(sys.modules[k], target, TransformerConfig)