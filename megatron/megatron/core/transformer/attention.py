...
from megatron.core.transformer.utils import _SeqAllToAll
...
class USPSelfAttention(SelfAttention):
    def __init__(
        ...
        assert self.usp_size > 1
        assert self.config.num_attention_heads % self.usp_size == 0

        transformer_engine_config = copy.deepcopy(self.config)
        transformer_engine_config.num_attention_heads = self.config.num_attention_heads // self.usp_size
        transformer_engine_config.num_query_groups = self.config.num_query_groups // self.usp_size
    ...
    def forward(
        ...
        query = _SeqAllToAll.apply(self.usp_group, query, scatter_idx=2, gather_idx=1)
        key = _SeqAllToAll.apply(self.usp_group, key, scatter_idx=2, gather_idx=1)
        value = _SeqAllToAll.apply(self.usp_group, value, scatter_idx=2, gather_idx=1)

        if not self.local_attn.is_bshd_format():
            # convert to sbhd 
            query, key, value = [x.transpose(0, 1).contiguous() for x in (query, key, value)]
        ...
        core_attn_out = _SeqAllToAll.apply(self.usp_group, core_attn_out, scatter_idx=1, getattr_idx=2).transpose(0, 1)
        core_attn_out = core_attn_out.view(output.shape[0], output.shape[1], -1)
