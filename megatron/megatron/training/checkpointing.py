...
def get_rng_state(use_dist_ckpt: bool = False):
    ...
    if use_dist_ckpt:
        ...
        rng_state_list = ShardedObject('rng_state', rng_state_list, (pp_size, tp_size), (pp_rank, tp_rank),
                                       replica_id=fs_ps.get_data_parallel_rank(with_context_parallel=True, with_ulysses_sp_parallel=True))
    ...
    if not torch.distributed.is_initialized() \
        ...
        if use_dist_ckpt:
            ...
            if checkpointing_context is not None and 'save_strategy' in checkpointing_context:
                ...
                if args.ckpt_fully_parallel_save:
                    save_strategy = FullyParallelSaveStrategyWrapper(save_strategy, fs_ps.get_data_parallel_group(with_context_parallel=True, with_ulysses_sp_parallel=True),
...
def _load_global_dist_base_checkpoint(
    ...
    if args.ckpt_fully_parallel_load:
        load_strategy = FullyParallelLoadStrategyWrapper(
            load_strategy, fs_ps.get_data_parallel_group(with_context_parallel=True, with_ulysses_sp_parallel=True)
        )
