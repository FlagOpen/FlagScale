...
class TaskEncoder(
    ...
    def __init__(self):
        ...
        # self.input_ids_logger = self._setup_input_ids_logger()


    # def _setup_input_ids_logger(self):
    #     """设置一个用于记录 input_id 的日志记录器，仅在 rank 0 上生效。"""
    #     logger = logging.getLogger('InputIDsLogger')
    #     # 只在主进程 (rank 0) 上配置日志记录器
    #     if self.args.rank == 0:
    #         logger.setLevel(logging.INFO)
    #         # 防止重复添加 handler
    #         if not logger.handlers:
    #             # 日志将输出到名为 'input_ids.log' 的文件中
    #             handler = logging.FileHandler('input_ids.log')
    ...
    def encode_chatml(self, sample: ChatMLSample):
        ...
        for turn_idx, turn in enumerate(conversation):
            ...
            if role == "human":
                ...
                if "<action_token>" in content:
                    ...
                    action_part = content
                    if "<boa>" in content:
                        # New format: "text<boa><action_token>"
                        parts = content.split("<boa>", 1)
                        text_before_boa = parts[0].strip()
                        action_part = parts[1] if len(parts) > 1 else ""
                        dataset_logger.debug(f"Sample [{sample.__key__}]: New format detected. Text before <boa>: '{text_before_boa}'")
                    else:
                        # Old format: "<action_token>" (no text before)
                        dataset_logger.debug(f"Sample [{sample.__key__}]: Old format detected.")
                    ...
                    if hasattr(sample, 'metadata') and isinstance(sample.metadata, dict):
                        action_token_paths = sample.metadata.get('action_eepose_token')
                    ...
                    if action_tokens_loaded and current_action_tokens:
                        if text_before_boa:
                            content = text_before_boa
                            dataset_logger.debug(f"Sample [{sample.__key__}]: Loaded {len(current_action_tokens)} action tokens, preserved text: '{content}'")
                        else:
                            content = ""
                            dataset_logger.debug(f"Sample [{sample.__key__}]: Loaded {len(current_action_tokens)} action tokens, cleared content")
        ...
        # if self.args.rank == 0:
        #     # 将完整的 numpy 数组转换为列表进行记录
        #     log_message = (
        #         f"样本 [{sample.__key__}] | "
        #         f"形状: {final_input_ids.shape} | "
        #         f"完整 Input IDs: {final_input_ids.tolist()}"
        #     )
        #     self.input_ids_logger.info(log_message)
