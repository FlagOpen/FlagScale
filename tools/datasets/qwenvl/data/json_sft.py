...
class JSONSFTDataset(torch.utils.data.Dataset):
    ...
    def preprocess(self, examples):
        ...
        if "output" in examples:
            ...
            raise ValueError(
                "Cannot find output key `output`, `content` or `response`!"
            )
        ...
        for data in zip(*datas):
            ...
            for t1, t2 in zip(source, full):
                assert (
                    t1 == t2
                ), "The user input_ids are not a prefix of the full input_ids! Please check the template."
            ...
            if len(full) > self.max_padding_length:
                ...
                assert (
                    full[-1] == self.eos_token_id
                ), f"Assume any untruncated sample ends with <eos>! But got: {self.tokenizer.detokenize(full)}"
                full[-1] = -1 - full[-1]

            if self.max_padding_length > len(full):
                full = full + [self.IGNORE_INDEX] * (
                    self.max_padding_length - len(full)
                )
