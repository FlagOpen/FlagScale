            ...
            if role == "human":
                ...
                if "<action_token>" in content:
                    ...
                    if hasattr(sample, 'metadata') and isinstance(sample.metadata, dict):
                        action_token_paths = sample.metadata.get('action_qpos_token')
                    ...
                    if action_tokens_loaded and current_action_tokens:
                        ...
                        should_have_action_tokens = (
                            ...
                            'action_qpos_token' in sample.metadata
                        )

                        if should_have_action_tokens:
                            # 样本应该有 action tokens 但加载失败
                            dataset_logger.error(
                                f"Sample [{sample.__key__}]: Failed to load action tokens despite having 'action_qpos_token' in metadata. "
                                f"File path might be incorrect: {sample.metadata.get('action_qpos_token')}. "
