...
class TaskEncoder(
    ...
    def encode_chatml(self, sample: ChatMLSample):
            ...
            if role == "human":
                ...
                if "<action_token>" in content:
                    ...
                    # if sample.imgs and len(sample.imgs) > 0:
                        ...
                        #     # action_token_filename = f'agilex_token_{frame_number}.npy'
                        ...
                        #     print(full_action_token_path)
                        if hasattr(sample, 'action_token') and sample.action_token:
                            full_action_token_path = sample.action_token
                            print("############")
                            print("full_action_token_path",full_action_token_path)
                            if os.path.exists(full_action_token_path):
                                try:
                                    loaded_tokens = np.load(full_action_token_path).flatten().tolist()
                                    # DEBUG PRINT: Show raw IDs from .npy file
                                    # print(f"DEBUG: Raw action tokens from {full_action_token_path}: {loaded_tokens}")
                                    
                                    # CORRECTED: Map relative action IDs to absolute vocabulary token IDs
                                    current_action_tokens = [int(token) for token in loaded_tokens]

                                    
                                    # CRITICAL CHANGE: Clear content to prevent it from being tokenized
                                    content = ""
                                except Exception as e:
        ...
        # print('final_input_ids[:100]:', final_input_ids[:100])
        # print('final_input_ids[-100:]:', final_input_ids[-100:])
