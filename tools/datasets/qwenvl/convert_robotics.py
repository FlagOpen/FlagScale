'''
dataset: /share/project/dumengfei/data/agilex_data_0905_F6A3C30/agilex_train_shuffle_prompt_v1.jsonl
...
def convert(
    ...
    video_key="videos",
    action_key="action",
    action_qpos_key="qpos",
    action_eepose_key="eepose",
    state_key="state",
    state_qpos_key="qpos",
    state_eepose_key="eepose",
    vision_dir=None,
    dp_size=1,
    drop_last=False,
):
    ...
    add_handlers(default_handlers, 'action_qpos', lambda data: pickle.dumps(data))
    add_handlers(default_handlers, 'action_eepose', lambda data: pickle.dumps(data))
    add_handlers(default_handlers, 'state_qpos', lambda data: pickle.dumps(data))
    add_handlers(default_handlers, 'state_eepose', lambda data: pickle.dumps(data))
    ...
    def write_sample(entry, vision_dir, has_idx=None, idx=0):
        ...
        action_qpos = action.get(action_qpos_key, "")
        action_eepose = action.get(action_eepose_key, "")
        state = entry.get(state_key, {})
        state_qpos = state.get(state_qpos_key, "")
        state_eepose = state.get(state_eepose_key, "")
        ...
        sample = {
            ...
            "videos": video_datas,
            "json": json.dumps(
                {"conversations": entry["converations"], "second_per_grid_ts": second_per_grid_ts}
            ).encode("utf-8"),
        }
...
if __name__ == "__main__":
    ...
    output_dir = convert(
        ...
        action_key=args.actions_key,
        state_key=args.state_key,
        action_qpos_key=args.actions_qpos_key,
        action_eepose_key=args.actions_eepose_key,
        state_qpos_key=args.state_qpos_key,
