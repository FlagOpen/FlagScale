...
class SupervisedWebDataset(IterableDataset):
    ...
    def _get_item(self, sources) -> Dict[str, torch.Tensor]:
        ...
        print(
            f'_get_item {current_time.strftime("%Y-%m-%d %H:%M:%S")} id={sources["id"]}',
            flush=True,
        )
...
def process_index_in_node(indexes, output_dir, data_module):
    num_process = 8 if len(indexes) > 8 else len(indexes)

    num_index_per_process = len(indexes) // num_process
    processes = []
    for i in range(num_process):
        start = i * num_index_per_process
        end = (
            (i + 1) * num_index_per_process if i != (num_process - 1) else len(indexes)
        )
        process_indexes = indexes[start:end]
        p = multiprocessing.Process(
            target=process_index, args=(process_indexes, output_dir, data_module, i)
        )
        processes.append(p)
        p.start()
...
def process_index(indexes, output_dir, data_module, process_id):
    rank = int(os.environ.get("OMPI_COMM_WORLD_RANK", 0))
    print(f"Node {rank} Process {process_id} is processing")
    train_dataset = data_module["train_dataset"]
    data_collator = data_module["data_collator"]
    with wds.ShardWriter(
        os.path.join(output_dir, f"llava-ov-node-{rank}-process-{process_id}-%d.tar"),
        maxcount=10000,
    ) as shard_writer:
        print(f"sample num: {len(indexes)}")
        global_id = 0
        for index in tqdm(indexes):
            entry = data_collator([train_dataset[index]])
            sequence = []
    ...
    print(f"rank {rank} datasets saved to {output_dir}")


def find_index_files(input_dir):
    index_files = []
    for root, dirs, files in os.walk(input_dir):
        for file in files:
            if file.endswith("indexes.pkl"):
...
def train(attn_implementation=None):
    ...
    index_files = find_index_files(output)
    num_index_files = len(index_files)
    assert num_index_files > size
    ...
    for index_file in index_files:
        ...
        with open(index_file, "rb") as f:
            ...
            all_indexes += indexes
    num_index = len(all_indexes)

    num_index_per_node = num_index // size
    start_index = rank * num_index_per_node
    end_index = (rank + 1) * num_index_per_node if rank != size - 1 else num_index
    node_indexes = all_indexes[start_index:end_index]
    process_index_in_node(node_indexes, output, data_module)
    end_time = time.time()
    print(f"Node {rank} done in {end_time-start_time:.2f}s")
