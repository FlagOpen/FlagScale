...
def patch(main_path, submodule_name, src, dst):
    ...
    for file_path in file_statuses:
        if file_statuses[file_path][0] == 'D':
            continue
        _sync(file_path, file_statuses[file_path], src, dst, tmp_file)
    ...
    if file_status_deleted:
        try:
            for file_path in file_status_deleted:
                ...
                _sync(file_path, file_status_deleted[file_path], src, dst, tmp_file)
...
def _sync(file_path, status, src, dst, f=None):
    ...
    if change_type == 'T':
        ...
        if is_symlink:
            ...
            _create_file_and_symlink(src_file_path, dst_file_path)
        ...
        if is_symlink:
            ...
            _create_file_and_symlink(renamed_src_file_path, renamed_dst_file_path)
            assert f
            f.write(f"{file_path}\n")
            f.flush()


def _create_file_and_symlink(source_file, target_file):
